...
function addLot() {
    ...
    const lotHTML = `
        <tr class="lot-row">
            ...
            <td class="action-cell"><button type="button" class="delete-btn" onclick="deleteRow(this)" title="Xóa dòng">×</button></td>
        </tr>
        <tr>
            <td><input type="text" class="item-input" name="size" placeholder="Size"></td>
            <td><input type="number" class="item-input" name="percent" step="0.01" placeholder="%"></td>
...
function addRowToCurrentLot() {
    ...
    if (totalRow) {
        ...
        const dataRowsCount = lotRows.filter(r => !r.classList.contains('total-row') && 
                                                 !r.classList.contains('phidn-row') && 
                                                 !r.classList.contains('thanhtien-row')).length;
        
        if (lotNumberCell) lotNumberCell.rowSpan = dataRowsCount;
        if (companyNameCell) companyNameCell.rowSpan = dataRowsCount;
        if (productTypeCell) productTypeCell.rowSpan = dataRowsCount;
...
// Xóa dòng cuối (giữ lại cho tương thích)
...
function deleteRow(button) {
    ...
    // Không cho phép xóa dòng tổng, phí DN, thành tiền
    if (row.classList.contains('total-row') || 
        row.classList.contains('phidn-row') || 
        row.classList.contains('thanhtien-row')) {
        alert('Không thể xóa dòng này!');
    ...
    // Kiểm tra nếu đây là dòng duy nhất của lô
    const dataRows = lotRows.filter(r => !r.classList.contains('total-row') && 
                                         !r.classList.contains('phidn-row') && 
                                         !r.classList.contains('thanhtien-row'));
    
    if (dataRows.length <= 1) {
        // Nếu chỉ còn 1 dòng, xóa cả lô
        if (confirm('Đây là dòng cuối cùng của lô. Bạn có muốn xóa cả lô?')) {
            deleteLot(row);
        }
        return;
    }
    ...
    if (confirm('Bạn có chắc chắn muốn xóa dòng này?')) {
        // Cập nhật rowspan trước khi xóa
        updateRowSpanAfterDelete(row);
        ...
        // Tính lại tổng
        setTimeout(() => {
            const remainingLotRows = getLotRows(lotRows[0]);
            if (remainingLotRows.length > 0) {
                calculateLotTotal(remainingLotRows[0].querySelector('input'));
                calculateAllTotals();
            }
        }, 100);
    }
...
function deleteLot(row) {
    ...
    // Kiểm tra nếu chỉ còn 1 lô
    const lotNumbers = new Set();
    allRows.forEach(r => {
        const lotCell = r.querySelector('.lot-number');
        if (lotCell) {
            lotNumbers.add(lotCell.textContent);
        }
    });
    
    if (lotNumbers.size <= 1) {
        alert('Không thể xóa lô cuối cùng!');
        return;
    }
    
    // Xóa tất cả dòng của lô
    lotRows.forEach(lotRow => {
        lotRow.remove();
    });
...
// Cập nhật rowspan sau khi xóa dòng
function updateRowSpanAfterDelete(rowToDelete) {
    const lotRows = getLotRows(rowToDelete);
    const firstRow = lotRows[0];
    ...
    if (firstRow) {
        ...
        const currentDataRows = lotRows.filter(r => !r.classList.contains('total-row') && 
                                                   !r.classList.contains('phidn-row') && 
                                                   !r.classList.contains('thanhtien-row'));
        
        const newRowSpan = currentDataRows.length - 1; // Trừ 1 vì sẽ xóa 1 dòng
        
        if (lotNumberCell) lotNumberCell.rowSpan = newRowSpan;
        if (companyNameCell) companyNameCell.rowSpan = newRowSpan;
        if (productTypeCell) productTypeCell.rowSpan = newRowSpan;
    }
}

// Cập nhật lại số lô sau khi xóa lô
function updateLotNumbers() {
    const tbody = document.getElementById('invoice-items');
    const lotCells = tbody.querySelectorAll('.lot-number');
    
    lotCells.forEach((cell, index) => {
        cell.textContent = index + 1;
    });
    
    currentLotNumber = lotCells.length;
}
