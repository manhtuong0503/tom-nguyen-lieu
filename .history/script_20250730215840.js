...
function initializeApp() {
    // Không cần set ngày hiện tại vì đã có sẵn trong HTML
    
    // Thiết lập sự kiện cho các input
    setupEventListeners();
    
    // Điền sẵn một số dữ liệu mẫu như trong phiếu
    fillSampleData();
}
...
function calculateTotal() {
    ...
    // Tiền thuế (để trống như trong mẫu)
    const tienThue = 0;
    ...
    updateMoneyInWords(tongThanhToan);
}
